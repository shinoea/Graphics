...
//Bresenham 的线段绘制算法
//1.确定线段的起点和终点坐标。
//2.计算线段的斜率 k = (y2 - y1) / (x2 - x1)。根据斜率的正负, 确定沿 x 轴还是 y 轴移动为主。
//3.定义一个决策变量 d, 初始值为 2 * (y2 - y1) - (x2 - x1)。这个变量决定下一步是往 x 轴还是 y 轴方向移动。
//4.从起点开始, 根据 d 的正负依次绘制每个像素点 :
//   如果 d < 0, 说明下一个像素点应该在 x 轴方向移动, 则 x 坐标加 1, d 加 2 * (y2 - y1)。
//   如果 d >= 0, 说明下一个像素点应该在 y 轴方向移动, 则 y 坐标加 1, d 加 2 * (y2 - y1) - 2 * (x2 - x1)。
//5.重复第 4 步, 直到达到终点。
...
auto to_vec4(const Eigen::Vector3f& v3, float w = 1.0f)//三维坐标点转齐次思维坐标点
{
    return Vector4f(v3.x(), v3.y(), v3.z(), w);
}

static bool insideTriangle(int x, int y, const Vector4f* _v){//判断某个点是否在三角形内部
    Vector3f v[3];
    for(int i=0;i<3;i++)
...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        for (auto & vert : v)
        {
            ...
            //vert.z() * f1 会将 z 坐标从 [-1, 1] 映射到 [near, far] 范围内。+ f2 则将结果偏移到正确的深度范围。
            // 为了Zbuffer保留Z值
    ...
    for (int x = min_x; x <= max_x; x++)
    {
        for (int y = min_y; y <= max_y; y++)
        {
            ...
            if (insideTriangle(x + 0.5, y + 0.5, t.v))
            {
                ...
                tie(alpha, beta, gamma) = tmp;

                //此时计算的重心坐标是二维下的，而计算深度应在三维下，所以需要进行修正
